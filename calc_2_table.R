...
calc_2_table<- function (table1, table2, operator, removeNa = TRUE) {
  ...
  if(operator == '+'){
    ...
    table4<- table3%>%mutate(!!sym(name3):= x)%>% 
      select(-c(name1,name2)) ##Use symbol variables & !! for dynamic naming
  }else if (operator =='/'){
    table4<- table3%>%
      mutate(!!sym(name3) := (!!sym(name1)) / (!!sym(name2)))%>% ##Use symbol variables & !! for dynamic naming
